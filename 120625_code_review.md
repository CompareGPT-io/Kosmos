...
#### Major Gaps (Should Fix Before Release): 2

1. **ArXiv Python 3.11+ Incompatibility**
   - **Severity**: Medium
   - **Impact**: Cannot use arXiv search on modern Python
   - **Workaround**: Semantic Scholar as fallback (already implemented)
   - **Recommended Fix**: Implement alternative ArXiv client without sgmllib
...
3. **Neo4j Storage Size** - Hardcoded 0 in stats
...
1. TODO in `simple.py:837` for Neo4j storage size
...
- [ ] Add ArXiv alternative client (without sgmllib)
- [ ] Complete Neo4j integration into main research loop
...
- ArXiv compatibility issue on Python 3.11+
